...
from .utils import write_ind_val_dsets, create_indexed_group, write_simple_attrs
from .utils import write_main_dataset

class IgorIBWTranslator(object):
    """
    ...
    def translate(self, file_path, verbose=False, grp_name='Measurement', parm_encoding='utf-8'):
        
        """
        Translates the provided file to IgorObject
        Parameters
        ----------
        ...
        igor_file = IgorFile(name=fname)
        # Create measurement group
        meas_grp = create_indexed_group(igor_file, grp_name)
        
        # Write file and measurement level parameters
        global_parms = dict()
        global_parms['object_type'] = 'IgorObj'
        global_parms['data_type'] = 'IgorIBW_' + type_suffix
        global_parms['translator'] = 'IgorIBW'

        write_simple_attrs(igor_file, global_parms)
        write_simple_attrs(meas_grp, parm_dict)

        # Create Position and spectroscopic datasets        
        pos_inds, pos_vals = write_ind_val_dsets(meas_grp, pos_desc, is_spectral=False, slow_to_fast=True)
        spec_inds, spec_vals = write_ind_val_dsets(meas_grp, spec_desc, is_spectral=True, slow_to_fast=True)  
        ...
        for chan_data, chan_name, chan_unit in zip(images, chan_labels, chan_units):
            ...
            chan_grp = create_indexed_group(meas_grp, base_name='Channel')
            
            write_main_dataset(chan_grp, np.atleast_2d(chan_data), 'Raw_Data',
                            chan_name, chan_unit,
                            None, None,
                            pos_inds=pos_inds, pos_vals=pos_vals,
                            spec_inds=spec_inds, spec_vals=spec_vals,
                            dtype=np.float32)
            write_ind_val_dsets(chan_grp['Raw_Data'], pos_desc, is_spectral=False, slow_to_fast=True)
            write_ind_val_dsets(chan_grp['Raw_Data'], spec_desc, is_spectral=True, slow_to_fast=True) 
        if verbose:
    ...
    def translate_aris(self, file_path, verbose=False):
        
        file_path = os.path.abspath(file_path)

        # Load the image stack using h5py
        # with h5py.File(file_path, 'r'):

def _byte2str(bytes, codec='utf-8'):
    if isinstance(bytes, list):
        strings = [b.decode(codec, errors='ignore') for b in bytes]
        return strings
    else:
        return bytes.decode(codec, errors='ignore')